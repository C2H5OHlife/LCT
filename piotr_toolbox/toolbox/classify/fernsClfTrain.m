...
if( isempty(ferns) )
  ...
  thrs=rand(M,S)*(thrr(2)-thrr(1))+thrr(1); % 像素点二元比较编程特征值和阈值的比较
  fids=uint32(floor(rand(M,S)*F+1)); % 为每个蕨随机选取S个特征
  inds=fernsInds(data,fids,thrs); % 为每个蕨计算每个样本的index inds:[NxM]
...
% 对每一个蕨中的每一类做直方图统计pFern
pFern = zeros(2^S,H,M); edges = 1:2^S;
for h=1:H, inds1=inds(hs==h,:);% 提取第h类的样本的index
  for m=1:M, pFern(:,h,m)=histc(inds1(:,m),edges); end %计算第m个蕨的第h类的直方图
end
pFern = pFern + bayes;

% convert fern leaf class counts into probabilities 归一化
if( bayes<=0 )
  norm = 1./sum(pFern,2);
