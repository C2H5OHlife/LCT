...
%   sets up the parameters and loads the video information.
    ...
    [window_sz, app_sz]=search_window(target_sz,im_sz, config); %更科学的加padding
    ...
    cell_size=config.features.cell_size; %HOG的cell大小
    interp_factor=config.interp_factor; %学习率
...
%     max_scale_factor = scale_step ^ floor(log(min([size(im,1) size(im,2)] ./ base_target_sz)) / log(scale_step));
	...
	for frame = 1:numel(img_files),
        ...
        if frame > 1,
            ...
            %motion_model确定位置，app_model确定最大响应
...
%           svm_struct=det_learn(im, pos, window_sz, config.detc, []); %检测器
            ferns=det_learn(im, pos, window_sz, config.detc, []); %重检测器训练
