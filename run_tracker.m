...
function [precision, fps] = run_tracker(video, show_visualization, show_plots)
    ...
    config.features.window_size = 6; % sizevlfeat-0.9.20 of local region for intensity historgram  
    config.features.nbins=8; % bins of intensity historgram
    
    config.motion_thresh=0.15; % 重检测阈值  
    config.appearance_thresh=0.38; %app_model更新阈值
